...
window['__$dd'] = async () => {
    ...
    const setDDProp = async (dd, propKey, asyncFunc) => {
        ...
        try {
            ...
            console.log('propKey:', propKey, ' droped because of exception', e)
        }

        if (!propValue) {
            console.log('propKey:', propKey, ' droped because of falsy resulty')
            return;
        }

        if ((propValue instanceof Array) && propValue.length === 0) {
            console.log('propKey:', propKey, ' droped because of empty array')
            return;
        }
