...
export default class DeviceDescriptorHelper {
    ...
    static buildFakeDeviceDescriptor(deviceDesc: DeviceDescriptor) : FakeDeviceDescriptor {
        assert(!!deviceDesc)

        const result: FakeDeviceDescriptor = _.clone(deviceDesc) as FakeDeviceDescriptor
        ...
        if (!result.fontSalt) {
            ...
            let fontEntries = result.allFonts
            ...
            result.fontSalt = fonts
        }

        // canvas with salt
        if (!result.canvasSalt || !result.canvasSalt.length) {
